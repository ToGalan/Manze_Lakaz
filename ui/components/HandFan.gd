...
func _layout() -> void:
	...
	# smaller than the original) -- 0.5x turned out too small. +10.0 flat
	# on top of that, in two +5px rounds per separate requests.
	var card_h: float = clamp(size.y * 0.90, 135.0, 285.0) * 0.75 + 10.0
