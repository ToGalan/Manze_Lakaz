...
## def_id here always names a normal (non-joker) ingredient: a joker can
## never be a legal STEAL target (RulesEngine never offers one -- see
## _add_take_actions()), and this is only ever called with the def_id of a
## card an actual STEAL action targets.
