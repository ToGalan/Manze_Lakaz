...
## No longer sizes the draft offer -- RulesEngine._offer_draft() now always
## offers one recipe per difficulty tier (Easy/Medium/Hard), so the real
## offer size tracks the recipe database's own tier count instead. Kept
## around for config/serialization compatibility and as a rough "how many
## recipes will a player see" figure for callers like the simulator.
