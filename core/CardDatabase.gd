...
func easy_recipe_ids() -> Array[String]:
	return _recipe_ids_of_tier(RecipeDef.Tier.EASY)

func medium_recipe_ids() -> Array[String]:
	return _recipe_ids_of_tier(RecipeDef.Tier.MEDIUM)

func hard_recipe_ids() -> Array[String]:
	return _recipe_ids_of_tier(RecipeDef.Tier.HARD)

func _recipe_ids_of_tier(tier: int) -> Array[String]:
	var out: Array[String] = []
	for id in recipe_order:
		if recipe_defs[id].tier == tier:
			out.append(id)
	return out
