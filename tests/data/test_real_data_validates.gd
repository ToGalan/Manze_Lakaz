...
func test_deck_totals_match_spec() -> void:
	...
	# 88 non-joker ingredient cards + 6 jokers (2 each of joker_protein/
	# joker_vegetable/joker_pantry) + 30 preparations = 124.
	TestUtil.assert_eq(deck.size(), 124, "the full deck should contain exactly 124 cards")
	...
	for c in deck:
		if c.is_ingredient():
			...
			preparation_count += 1
	TestUtil.assert_eq(ingredient_count, 94, "94 ingredient cards expected in the deck (88 original + 6 jokers)")
	TestUtil.assert_eq(preparation_count, 30, "30 preparation cards expected in the deck")
