...
## engine guessing one. Once attached, a joker still counts toward recipe
## completion exactly like any other filled ingredient card, but -- unlike a
## normal ingredient -- it can never be stolen back off the recipe, same as
## a preparation.
...
func test_joker_attached_ingredient_is_never_stealable() -> void:
	var db := TestFixtures.joker_database()
	var config := TestFixtures.default_config()
	...
	# A normal ingredient on the same (still incomplete) recipe stays
	# stealable -- only jokers get this protection.
	...
	TestUtil.assert_false(steal_targets.has(joker_card.instance_id), "a joker filling a slot should never be offered as a steal target")
	TestUtil.assert_true(steal_targets.has(oil_card.instance_id), "a normal ingredient on the same recipe should still be stealable")
